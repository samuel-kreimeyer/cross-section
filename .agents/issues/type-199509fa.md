# Issue: Type Error: Unsupported operand types for / ("None" and "float...

**Type:** type
**Severity:** error
**Tool:** check-types
**Detected:** 2026-01-10T16:23:09.116091Z

## Summary
Type error detected by mypy on line 173.

## Evidence
File: src/cross_section/core/domain/components/slopes.py
Line: 173

mypy output:
```
src/cross_section/core/domain/components/slopes.py:173: error: Unsupported operand types for / ("None" and "float")  [operator]
```

## Impact
Type errors can lead to runtime failures. Static type checking helps catch these issues early.

## Recommended Action
Review the type error and fix the type annotation or adjust the code to match the expected type.

## Automation
- Detectable: yes
- Auto-fixable: no

## Metadata
```json
{
  "files": ["src/cross_section/core/domain/components/slopes.py"]
  "lines": [173]
}
```